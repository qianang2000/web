...
function calculateSubtotal() {
	...
	for (let i = 0; i < ITEMS.length; i++) {
		let item = ITEMS[i];
		const quantity = parseInt(document.getElementById(item.concat("-quantity")).value);
        const price = parseFloat(document.getElementById(item.concat("-price")).innerText);
...
function calculateSalesTax() {
	...
	let price = calculateSubtotal() * getSalesTaxRateForState(state);
